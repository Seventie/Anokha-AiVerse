...
const InterviewModule: React.FC = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0b1714] text-white">
      {currentView !== 'menu' && (
        <header className="w-full border-b border-white/10 px-6 md:px-12 py-4 flex items-center justify-between">
          <button
      ...
      {currentView === 'menu' && (
        <section className="min-h-screen flex flex-col justify-center px-8 md:px-24 pt-24">
          <div className="max-w-[1400px] mx-auto hero-content space-y-16">
            ...
            <p className="text-[11px] text-white/40 tracking-[0.3em] uppercase">
              Mock Interview Engine • Adviceguide
            </p>
          </div>
        </section>
      )}
      ...
      {currentView === 'setup' && (
        <InterviewSetup onInterviewCreated={handleInterviewCreated} onCancel={handleBackToMenu} />
      )}
      ...
      {currentView === 'results' && currentInterviewId && (
        <InterviewResults interviewId={currentInterviewId} onBackHome={handleBackToMenu} />
      )}

      {currentView === 'analytics' && (
        <div className="px-8 md:px-24 py-16 max-w-[1400px] mx-auto">
